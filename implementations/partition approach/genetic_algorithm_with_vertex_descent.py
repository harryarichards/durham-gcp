...
def selection(population):
    temp_parent_1 = [random.choice(population), random.choice(population)]
...
def conflicts_in_colour(vertex, colour):
    neighbours = graph.neighbors(vertex)
    return len(colour.intersection(neighbours))

def vertex_descent(child):
    min_conflicts = list()
    min_conflicts.append(number_of_conflicts(child))
    unchanged = False
    chosen_child = copy.deepcopy(child)
    while not unchanged:
        cost_of_colours = list()
        for vertex in graph.nodes:
            for update_vertex in graph.nodes:
                cost_of_colours.append([conflicts_in_colour(update_vertex, colour) for colour in child])

            for colour in child:
                if vertex in colour:
                    colour.remove(vertex)
            index_of_min = cost_of_colours[vertex-1].index(min(cost_of_colours[vertex-1]))
            child[index_of_min].add(vertex)
        min_conflicts.append(number_of_conflicts(child))
        if number_of_conflicts(child) <= number_of_conflicts(chosen_child):
            chosen_child = copy.deepcopy(child)
        if len(min_conflicts) > 100:
            if min_conflicts[len(min_conflicts)-1] == min_conflicts[len(min_conflicts)-100]:
                unchanged = True
    return chosen_child
...
def genetic_algorithm(number_of_colours):
    ...
    while terminate == False:
        ...
        if least_faults == 0:
            ...
            return len(best_colouring), terminate, best_colouring
